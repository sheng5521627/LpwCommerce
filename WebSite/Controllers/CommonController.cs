...
using WebSite.Models.Catalog;
using WebSite.Models.Common;
using WebSite.Models.Topics;
...
namespace WebSite.Controllers
{
    public class CommonController : BasePublicController
    {
        ...
        protected virtual int GetUnreadPrivateMessages()
        {
            ...
            {
                var privateMessages = _forumservice.GetAllPrivateMessages(_storeContext.CurrentStore.Id, 0, customer.Id, false, null, false, string.Empty, 0, 1);
        ...
        public ActionResult SetLanguage(int langid, string returnUrl = "")
        {
            ...
            if (_localizationSettings.SeoFriendlyUrlsForLanguagesEnabled)
            {
                ...
                returnUrl = returnUrl.AddLanguageSeoCodeToRawUrl(applicationPath, language);
            }
        ...
        {
            var avalibleCurrencies = _cacheManager.Get(string.Format(ModelCacheEventConsumer.AVAILABLE_CURRENCIES_MODEL_KEY, _workContext.WorkingLanguage.Id, _storeContext.CurrentStore.Id), () =>
            {
                var result = _currencyService.GetAllCurrencies(storeId: _storeContext.CurrentStore.Id)
                .Select(c =>
                {
                    var currencySymbol = "";
            ...
            var model = new CurrencySelectorModel()
            {
                ...
                AvailableCurrencies = avalibleCurrencies
            };

            if (model.AvailableCurrencies.Count == 0)
                return Content("");
        ...
        public ActionResult ContactUsSend(ContactUsModel model, bool captchaValid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_commonSettings.UseSystemEmailForContactUsForm)
                {
                    ...
                    body = string.Format("<strong>From</strong>: {0} - {1}<br /><br />{2}", Server.HtmlEncode(fromName), Server.HtmlEncode(email), body);
                }
                else
                {
                    from = email;
        ...
        public ActionResult Sitemap()
        {
            ...
            var cachedModel = _cacheManager.Get(cacheKey, () =>
            {
                ...
                if (_commonSettings.SitemapIncludeCategories)
                {
                    ...
                    model.Categories = categories.Select(m => m.ToModel()).ToList();
                }
